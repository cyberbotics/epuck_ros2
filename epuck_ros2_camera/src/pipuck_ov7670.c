...
int pipuck_ov7670_init(void)
{
    ...
    if (write_i2c(REG_COM7, 0x80) < 0)
    { // Reset to default values.
        return -1;
    }
    usleep(200000);
    if (write_i2c(REG_CLKRC, 0x80) < 0)
    { // No internal clock prescaler.
        return -1;
    }
    if (write_i2c(REG_TSLB, 0x04) < 0)
    { // 0x04 = 0x0c (default) but with YUYV
        return -1;
    }
    if (write_i2c(REG_COM7, COM7_YUV | COM7_FMT_VGA) < 0)
    { // Output format: YUV, VGA.
        return -1;
    }
    ...
    if (write_i2c(0xb0, 0x84) < 0)
    { // Color mode?? (Not documented!)
        return -1;
    }

    if (write_i2c(REG_HSTART, 0x13) < 0)
    { // start = HSTART<<3 + HREF[2:0] = 19*8 + 6 = 158
        return -1;
    }
    if (write_i2c(REG_HSTOP, 0x01) < 0)
    { // stop = HSTOP<<3 + HREF[5:3] = 1*8 + 6 = 14 (158+640-784)
        return -1;
    }
    if (write_i2c(REG_HREF, 0x36) < 0)
    { // With flag "edge offset" set, then the image is strange (too much clear, not sharp); so clear this bit.
        return -1;
    }
    if (write_i2c(REG_VSTART, 0x02) < 0)
    { // start = VSTART<<2 + VREF[1:0] = 2*4 + 2 = 10
        return -1;
    }
    if (write_i2c(REG_VSTOP, 0x7a) < 0)
    { // stop = VSTOP<<2 + VREF[3:2] = 122*4 + 2 = 490
        return -1;
    }
    ...
    if (write_i2c(0x2a, 0x00) < 0)
    { // Dummy pixels MSB
        return -1;
    }
    if (write_i2c(0x2b, 0x5B) < 0)
    { // Dummy pixels LSB
        return -1;
    }
    if (write_i2c(0x92, 0x22) < 0)
    { // Dummy rows LSB
        return -1;
    }
    if (write_i2c(0x93, 0x01) < 0)
    { // Dummy rows LSB
        return -1;
    }
